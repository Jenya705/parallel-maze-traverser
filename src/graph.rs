...
use crate::{
    bfs::{handle_single_state, validate_capacity},
    delta_list::{BitSetDeltaList, DeltaList},
    end_state,
    instructions::collect_instructions4d,
    Coordinate, Map,
};
...
pub fn graph<const RESPECT_HOLES: bool>(
    ...
    gen_graph::<_, RESPECT_HOLES>(
        ...
        height,
        maps,
        &collect_instructions4d::<RESPECT_HOLES>(
...
pub fn gen_graph<W: io::Write, const RESPECT_HOLES: bool>(
    ...
    while !tasks.is_empty() {
        validate_capacity(&tasks, &mut output);

        for state in tasks.drain(..) {
            let l = output.len();

            // called validate_capacity and all states are generated by the following function
            unsafe {
